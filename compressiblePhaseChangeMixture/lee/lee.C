...
Foam::compressiblePhaseChangeMixtures::lee::mDot()
{
    ...
    //const volScalarField& Tsat_ = mesh_.lookupObject<volScalarField>("Tsat");
    ...
    (
        coeffE_*rho1*limitedAlpha1*max(T.oldTime() - Tsat_, T0),
        coeffC_*rho2*limitedAlpha2*max(Tsat_ - T.oldTime(), T0)
    );
}
