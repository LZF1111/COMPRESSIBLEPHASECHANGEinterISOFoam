...
License
    ...
    for more details.recoded in IPEZKZS
...
:
  ...
  /*  Tsat_
   ...
   hf_
    ...
    Tsat_(dimensionedScalar("Tsat",dimTemperature,subDict("phaseChangeProperties").lookup("Tsat")))
    
    //TsatConstant_(dimensionedScalar("TsatConstant",dimTemperature,subDict("phaseChangeProperties").lookup("TsatConstant")))
{
...
Foam::compressiblePhaseChangeMixture::updatedTsat()
{
    Tsat_= dimensionedScalar("fixedTsat", dimTemperature, 373.0);
    volScalarField& Tsat = this->Tsat_;
    // 对于边界，可以根据需要进行设置
    Tsat.correctBoundaryConditions();
    Info<<"Tsat set to constant value: "<<max(Tsat).value()<<endl;
    return Tsat;
}*/
